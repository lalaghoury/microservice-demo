...
export class ProductController {
    ...
    @Put(':id')
    update(@Param('id') id: string, @Body() updateProductDto: Partial<ProductMessage>) {
    ...
    async decreaseQuantity(data: DecreaseQuantityRequest): Promise<DecreaseQuantityResponse> {
        const success = await this.productService.decreaseQuantity(data.id, data.quantity);
        return { success };
    }
}
